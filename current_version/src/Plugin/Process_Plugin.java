...
{
	public static final String myClassName = "Process_Plogin";
	...
	public Process_Plugin(Plugin par, String PLUGIN_NAME, String PLUGIN_DESCRIPTION, String execution_command, boolean SET_CANONICAL_PATHS)
	{
		try
		{
			parent = par;
	...
	public boolean execute_command()
	{
		try
		{
			...
			{
				//if(!cmd.contains(Interface.fle_volatility.getCanonicalPath().trim()))
				...
				//if(!cmd.contains(Interface.fle_memory_image.getCanonicalPath().trim()))
			...
			File fleOutput = new File(Interface.path_fle_analysis_directory + this.plugin_name + File.separator + "_" + plugin_name + "_" + time_stamp + ".txt");
...
//				}
			...
			{				
				for(File fle : listing)
				{
					...
					//otw, process file					
					FileAttributeData attr = new FileAttributeData(fle);
					attr.set_hash(false);
					pwOut.println(attr.get_attributes("\t  "));
...
//				
	...
	public boolean process_plugin_mftparser(LinkedList<String> output, PrintWriter pw, String delimiter, File fleOutput)
	{
		try
		{
			...
			String MFT_HEADER = "Creation Date" + delimiter + "Creation Time" + delimiter + "Creation UTC" + delimiter + "Modified Date" + delimiter + "Modified Time" + delimiter + "Modified UTC" + delimiter + "MFT Altered Date" + delimiter + "MFT Altered Time" + delimiter + "MFT Altered UTC" + delimiter + "Access Date" + delimiter + "Access Time" + delimiter + "Access UTC" + delimiter + "Type/Name/Path" + delimiter + "Entry Atrribute" + delimiter + "Extension";
			...
			LinkedList<String> list_prefetch = new LinkedList<String>();
			for(String line : output)
			{
				if(line == null)
					continue;
