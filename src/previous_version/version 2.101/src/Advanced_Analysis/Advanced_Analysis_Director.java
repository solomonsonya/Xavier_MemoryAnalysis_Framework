	...
	public boolean import_advanced_analysis(LinkedList<File> list, File directory)
	{
		...
		try
		{
			...
			for(File fle : list)
			{
				...
				{
					plugin_envars = new Analysis_Plugin_envars(fle, this, "envars", "Display process environment variables", false);
			...
			for(File fle : list)
			{
				...
				if(!fle_name.endsWith(".txt"))
				 ...
				 if(fle_name.startsWith("dlllist"))
					plugin_dlllist = new Analysis_Plugin_dlllist(fle,this, "dlllist", "Print list of loaded dlls for each process", false);
				 
				//else if(fle_name.startsWith("moddump"))
					//plugin_moddump = new Analysis_Plugin_SUPER_MODULES(fle,this, "moddump", "Dump a kernel driver to an executable file sample", false); //runs modules and modscan within moddump!
					
				else if(fle_name.startsWith("modules"))
					plugin_modules = new Analysis_Plugin_modules(fle, this, "modules", "Print list of loaded modules", false); //run modules and modscan within moddump!
				
				else if(fle_name.startsWith("modscan"))
					plugin_modscan = new Analysis_Plugin_modscan(fle, this, "modscan", "Pool scanner for kernel modules", false); 
				
				else if(fle_name.startsWith("drivermodule"))
					plugin_drivermodule = new Analysis_Plugin_drivermodule(fle, this, "drivermodule", "Associate driver objects to kernel modules", false);
				
				else if(fle_name.startsWith("driverscan"))
					plugin_driverscan = new Analysis_Plugin_driverscan(fle, this, "driverscan", "Pool scanner for driver objects", false);
							
				else if(fle_name.startsWith("driverirp"))
					plugin_driverirp = new Analysis_Plugin_driverirp(fle, this, "driverirp", "Driver IRP hook detection", false);
												
				else if(fle_name.startsWith("timers"))
					plugin_timers  = new Analysis_Plugin_Timers(fle, this, "timers", "Print kernel timers and associated module DPCs", false);				 
					
				else if(fle_name.startsWith("ldrmodules"))
					plugin_ldrmodules = new Analysis_Plugin_ldrmodules(fle,this, "ldrmodules", "Detect unlinked DLLs", false);				
			...
			for(File fle : list)
			{
				...
				else if(fle_name.startsWith("impscan_"))
				{
					try
					{
						...
						{
							process.impscan = new Analysis_Plugin_impscan(fle, this, "impscan", "Scan for calls to imported functions", Analysis_Plugin_impscan.EXECUTE_VIA_THREAD, process);
						}
					}
					catch(Exception e)
					{
				...
				///////////////////////////////////////////////////////////////////////////
					...
					plugin_callbacks  = new Analysis_Plugin_Callbacks(fle, this, "callbacks", "Print system-wide notification routines", false); 
				
				else if(fle_name.startsWith("unloadedmodules"))
					plugin_unloaded_modules  = new Analysis_Plugin_Unloaded_Modules(fle, this, "unloadedmodules", "Print list of unloaded modules", false); 
				
				else if(fle_name.startsWith("hashdump"))				
					plugin_hashdump = new Analysis_Plugin_hashdump(fle, this, "hashdump", "Dumps passwords hashes (LM/NTLM) from memory", false);				
				
				else if(fle_name.startsWith("getservicesids"))
					plugin_get_service_sids = new Analysis_Plugin_get_service_sids(fle, this, "getservicesids", "Get the names of services in the Registry and return Calculated SID", false);
...
//else if(fle_name.startsWith("mftparser"))
	//plugin_mftparser = new Process_Plugin(fle, "mftparser", "Scans for and parses potential MFT entries", null, null, this.fle_volatility.getName() + " -f " + this.fle_memory_image.getName() + " mftparser --profile=" + this.PROFILE, true, false, "", false);
	
				//else if(fle_name.startsWith("registry_startup"))
				//	plugin_registry_startup = new Analysis_Plugin_Registry_Startup_Apps(fle,this, "registry_startup", "Print Subkeys of Registry Startup Locations", false);
					
				else if(fle_name.startsWith("svcscan"))
					plugin_svcscan = new Analysis_Plugin_svcscan(fle,this, "svcscan", "Scan for Windows services", false);
				
				else if(fle_name.startsWith("threads"))
					plugin_threads = new Analysis_Plugin_Threads(fle,this, "threads", "Investigate _ETHREAD and _KTHREADs", false);	
					
				else if(fle_name.startsWith("apihooks"))
					plugin_apihooks = new Analysis_Plugin_apihooks(fle,this, "apihooks", "Detect API hooks in process and kernel memory", false);
				
				else if(fle_name.startsWith("privs"))
					plugin_privs = new Analysis_Plugin_Privs(fle,this, "privs", "Display process privileges", false);
...
//else if(fle_name.startsWith("dlldump"))
	//dlldump = new Analysis_Plugin_Dump(fle,this, "dlldump", "Dump DLLs from a process address space", true, false);
					...
					plugin_cmdscan = new Analysis_Plugin_cmdscan(fle,this, "cmdscan", "Extract command history by scanning for _COMMAND_HISTORY", false);
			 				 
				else if(fle_name.startsWith("malfind"))
					plugin_malfind = new Analysis_Plugin_malfind(fle,this, "malfind", "Find hidden and injected code", false);
					
				else if(fle_name.startsWith("getsids"))
					plugin_getsids = new Analysis_Plugin_getsids(fle,this, "getsids", "Print the SIDs owning each process", false);
					
				else if(fle_name.startsWith("hivelist"))
					plugin_hivelist = new Analysis_Plugin_hivelist(fle,this, "hivelist", "Print list of registry hives", false);
					
				else if(fle_name.startsWith("hivescan"))
					plugin_hivescan = new Analysis_Plugin_EXECUTION(fle,this, "hivescan", "Pool scanner for registry hives", false);
					
				else if(fle_name.startsWith("dumpregistry"))
					plugin_regdump = new Analysis_Plugin_EXECUTION(fle,this, "dumpregistry", "Dumps registry files out to disk", false);
					
				else if(fle_name.startsWith("deskscan"))
					plugin_deskscan = new Analysis_Plugin_Deskscan(fle,this, "deskscan", "Poolscaner for tagDESKTOP (desktops)", false);
					
				else if(fle_name.startsWith("evtlogs"))
					plugin_evtlogs = new Analysis_Plugin_EXECUTION(fle,this, "evtlogs", "Extract Windows Event Logs (XP/2003 only)", false);
					
				else if(fle_name.startsWith("dumpfiles --regex"))
					plugin_evtlogs = new Analysis_Plugin_EXECUTION(fle,this, "dumpfiles - event logs", "Dump Windows Event Logs ", false);
					
				else if(fle_name.startsWith("filescan"))
					plugin_filescan = new Analysis_Plugin_EXECUTION(fle,this, "filescan", "Pool scanner for file objects", false);
					
				else if(fle_name.startsWith("gditimers"))
					plugin_gditimers = new Analysis_Plugin_GDI_Timers(fle,this, "gditimers", "Print installed GDI timers and callbacks", false);
					
				else if(fle_name.startsWith("imageinfo"))
					plugin_imageinfo = new Analysis_Plugin_EXECUTION(fle,this, "imageinfo", "Identify information for the image", false);
					
				else if(fle_name.startsWith("kdbgscan"))
					plugin_kdbgscan = new Analysis_Plugin_EXECUTION(fle,this, "kdbgscan", "Search for and dump potential KDBG values", false);
					
				else if(fle_name.startsWith("lsadump"))
					plugin_lsadump = new Analysis_Plugin_EXECUTION(fle,this, "lsadump", "Dump (decrypted) LSA secrets from the registry", false);
					
				else if(fle_name.startsWith("messagehooks"))
					plugin_messagehooks = new Analysis_Plugin_EXECUTION(fle,this, "messagehooks", "List desktop and thread window message hooks", false);
					
				else if(fle_name.startsWith("joblinks"))
					plugin_joblinks = new Analysis_Plugin_EXECUTION(fle,this, "joblinks", "Print process job link information", false);
				
				else if(fle_name.startsWith("notepad"))
					plugin_notepad = new Analysis_Plugin_EXECUTION(fle,this, "notepad", "List currently displayed notepad text", false);
					
				else if(fle_name.startsWith("sessions"))
					plugin_sessions = new Analysis_Plugin_sessions(fle,this, "sessions", "List details on _MM_SESSION_SPACE (user logon sessions)", false);
					
				else if(fle_name.startsWith("ssdt"))
					plugin_ssdt = new Analysis_Plugin_EXECUTION(fle,this, "ssdt", "Display SSDT entries", false);
					
				else if(fle_name.startsWith("symlinkscan"))
					plugin_symlinkscan = new Analysis_Plugin_EXECUTION(fle,this, "symlinkscan", "Pool scanner for symlink objects", false);
					
				else if(fle_name.startsWith("thrdscan"))
					plugin_thrdscan = new Analysis_Plugin_EXECUTION(fle,this, "thrdscan", "Pool scanner for thread objects", false);
					
				else if(fle_name.startsWith("userassist"))
					plugin_userassist = new Analysis_Plugin_user_assist(fle,this, "userassist", "Print userassist registry keys and information", false);
					
				else if(fle_name.startsWith("printkey"))
					plugin_printkey = new Analysis_Plugin_print_key(fle,this, "printkey", "Print a registry key, and its subkeys and values", false);
					
				else if(fle_name.startsWith("userhandles"))
					plugin_userhandles = new Analysis_Plugin_EXECUTION(fle,this, "userhandles", "Dump the USER handle tables", false);
					
				else if(fle_name.startsWith("vadinfo"))
					plugin_vadinfo = new Analysis_Plugin_VAD_INFO(fle,this, "vadinfo", "Dump the VAD info", false);
					
				else if(fle_name.startsWith("vadtree") && fle_name.endsWith(".txt"))
					plugin_vadtree = new Analysis_Plugin_vadtree(fle,this, "vadtree", "vadtree", false);
					
				else if(fle_name.startsWith("vadwalk"))
					plugin_vadwalk = new Analysis_Plugin_EXECUTION(fle,this, "vadwalk", "Walk the VAD tree", false);
					
				else if(fle_name.startsWith("win10cookie"))
					plugin_win10cookie = new Analysis_Plugin_EXECUTION(fle,this, "win10cookie", "Find the ObHeaderCookie value for Windows 10", false);
					
				else if(fle_name.startsWith("windows_"))
					plugin_windows = new Analysis_Plugin_EXECUTION(fle,this, "windows", "Print Desktop Windows (verbose details)", false);
										
				else if(fle_name.startsWith("wintree"))
					plugin_wintree = new Analysis_Plugin_EXECUTION(fle,this, "wintree", "Print Z", false);
					
				else if(fle_name.startsWith("wndscan"))
					plugin_wndscan = new Analysis_Plugin_EXECUTION(fle,this, "wndscan", "Pool scanner for window stations", false);
					
				else if(fle_name.startsWith("shellbags"))
					plugin_shellbags = new Analysis_Plugin_EXECUTION(fle,this, "shellbags", "Prints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags info", false);
					
				else if(fle_name.startsWith("shimcache"))
					plugin_shimcache = new Analysis_Plugin_EXECUTION(fle,this, "shimcache", "Parses the Application Compatibility Shim Cache registry key", false);
					
				else if(fle_name.startsWith("timeliner"))
					plugin_timeliner = new Analysis_Plugin_EXECUTION(fle,this, "timeliner", "Creates a timeline from various artifacts in memory", false);

					
				else if(fle_name.startsWith("verinfo"))
					plugin_verinfo = new Analysis_Plugin_verinfo(fle,this, "verinfo", "Prints out the version information from PE images", false);
					
				else if(fle_name.startsWith("cmdline"))
					plugin_cmdline = new Analysis_Plugin_cmdline(fle,this, "cmdline", "Display process command", false);
					
				else if(fle_name.startsWith("consoles"))
					plugin_consoles = new Analysis_Plugin_consoles(fle,this, "consoles", "Extract command history by scanning for _CONSOLE_INFORMATION", false);
					
				else if(fle_name.startsWith("connections"))
					plugin_connections = new Analysis_Plugin_connections(fle,this, "connections", "Print list of open connections [Windows XP and 2003 Only]", false);
					
				else if(fle_name.startsWith("connscan"))
					plugin_connscan = new Analysis_Plugin_connscan(fle,this, "connscan", "Pool scanner for tcp connections", false);
					
				else if(fle_name.startsWith("sockets"))
					plugin_sockets = new Analysis_Plugin_sockets(fle,this, "sockets", "Print list of open sockets", false);
					
				else if(fle_name.startsWith("sockscan"))
					plugin_sockscan = new Analysis_Plugin_sockscan(fle,this, "sockscan", "Pool scanner for tcp socket objects", false);
					
				else if(fle_name.startsWith("netscan"))
					plugin_netscan = new Analysis_Plugin_netscan(fle,this, "netscan", "Scan a Vista (or later) image for connections and sockets", false);
					
				else if(fle_name.startsWith("handles"))
					plugin_handles = new Analysis_Plugin_handles(fle,this, "handles", "Print list of open handles for each process", false);
					
				else if(fle_name.startsWith("handles"))
					plugin_audit_policies = new Analysis_Plugin_Auditpolicy(fle, this, "handles", "Prints out the Audit Policies from HKLM\\SECURITY\\Policy\\PolAdtEv", false);
				
				else if(fle_name.startsWith("shutdowntime"))
					plugin_shutdowntime = new Analysis_Plugin_ShutdownTime(fle, this, "shutdowntime", "Print ShutdownTime of machine from registry", false);
	...
	public boolean plugin_autorun()
	{
		...
		try
		{
			...
			driver.sop("Initiating autorun actions - building proceslist first...");
			//run processlists
			...
			for(String plugin : this.list_auto_plugins_execution)
			{
				try
				{
					...
					driver.sop("Executing --> " + plugin);
					
					//instantiate plugin
					Analysis_Plugin_EXECUTION PLUGIN = new Analysis_Plugin_EXECUTION(null, this, plugin, description, true);										
				}
		...
		{
			driver.eop(myClassName, "plugin_autorun", e);
		}
...
//delete/////////////////////////
//plugin_envars = new Analysis_Plugin_envars(null, this, "envars", "Display process environment variables", false);
...
if(false)
{
	...
	plugin_dlllist = new Analysis_Plugin_dlllist(null, this, "dlllist", "Print list of loaded dlls for each process", false);
	
	dlldump = new Analysis_Plugin_Dump(null, this, "dlldump", "Dump DLLs from a process address space", false, false);	
	
	procdump = new Analysis_Plugin_Dump(null, this, "procdump", "Dump a process to an executable file sample", false, false);
	
	plugin_moddump = new Analysis_Plugin_SUPER_MODULES(null, this, "moddump", "Dump a kernel driver to an executable file sample", false); //runs modules and modscan within moddump!
...
//end delete
			...
			/////////////////////////////////////////////////////////////////////			
			plugin_hashdump = new Analysis_Plugin_hashdump(null, this, "hashdump", "Dumps passwords hashes (LM/NTLM) from memory", true);
			plugin_get_service_sids = new Analysis_Plugin_get_service_sids(null, this, "getservicesids", "Get the names of services in the Registry and return Calculated SID", true);
			...
			plugin_registry_startup = new Analysis_Plugin_Registry_Startup_Apps(null, this, "registry_startup", "Print Subkeys of Registry Startup Locations", true);
...
//procdump = new Analysis_Plugin_Dump(null, this, "procdump", "Dump a process to an executable file sample", true, false);		
				...
				//service scan
				//
				plugin_svcscan = new Analysis_Plugin_svcscan(null, this, "svcscan", "Scan for Windows services", true);
				
				//
				//Threads
				//
				plugin_threads = new Analysis_Plugin_Threads(null, this, "threads", "Investigate _ETHREAD and _KTHREADs", true);	
				
				//
				//apihooks
				//
				plugin_apihooks = new Analysis_Plugin_apihooks(null, this, "apihooks", "Detect API hooks in process and kernel memory", true);
				
				//
				//Privs - thread
				//
				plugin_privs = new Analysis_Plugin_Privs(null, this, "privs", "Display process privileges", true);
			...
			sop("\nBuilding dll listing...");
				...
				//LDRMODULES
				//
				plugin_ldrmodules = new Analysis_Plugin_ldrmodules(null, this, "ldrmodules", "Detect unlinked DLLs", false);
...
procdump = new Analysis_Plugin_Dump(null, this, "procdump", "Dump a process to an executable file sample", true, false);
dlldump = new Analysis_Plugin_Dump(null, this, "dlldump", "Dump DLLs from a process address space", true, false);
plugin_cmdscan = new Analysis_Plugin_cmdscan(null, this, "cmdscan", "Extract command history by scanning for _COMMAND_HISTORY", true);
plugin_moddump = new Analysis_Plugin_SUPER_MODULES(null, this, "moddump", "Dump a kernel driver to an executable file sample", false); //runs modules and modscan within moddump!

				//////////////////////////////////////////////////////////////////////
				// Instantiate Worker Thread 
				/////////////////////////////////////////////////////////////////////
				plugin_malfind = new Analysis_Plugin_malfind(null, this, "malfind", "Find hidden and injected code", true);
				plugin_getsids = new Analysis_Plugin_getsids(null, this, "getsids", "Print the SIDs owning each process", true);
				//plugin_get_service_sids = new Analysis_Plugin_get_service_sids(null, this, "getservicesids", "Get the names of services in the Registry and return Calculated SID", true);
				plugin_hivelist = new Analysis_Plugin_hivelist(null, this, "hivelist", "Print list of registry hives", true);
				plugin_hivescan = new Analysis_Plugin_EXECUTION(null, this, "hivescan", "Pool scanner for registry hives", true);
...
plugin_regdump = new Analysis_Plugin_EXECUTION(null, this, "dumpregistry", "Dumps registry files out to disk", true);
plugin_deskscan = new Analysis_Plugin_Deskscan(null, this, "deskscan", "Poolscaner for tagDESKTOP (desktops)", true);
//plugin_drivermodule = new Analysis_Plugin_EXECUTION(null, this, "drivermodule", "Associate driver objects to kernel modules", true);
//plugin_driverscan = new Analysis_Plugin_EXECUTION(null, this, "driverscan", "Pool scanner for driver objects", true);

if(PROFILE.toLowerCase().contains("xp") || PROFILE.toLowerCase().contains("2003"))
	plugin_evtlogs = new Analysis_Plugin_EXECUTION(null, this, "evtlogs", "Extract Windows Event Logs (XP/2003 only)", true);
else if(PROFILE.toLowerCase().contains("win") || PROFILE.toLowerCase().contains("vista"))
	plugin_evtlogs = new Analysis_Plugin_EXECUTION(null, this, "dumpfiles --regex .evtx$ --ignore-case", "Dump Windows Event Logs ", true);
				
plugin_filescan = new Analysis_Plugin_EXECUTION(null, this, "filescan", "Pool scanner for file objects", true);
plugin_gditimers = new Analysis_Plugin_GDI_Timers(null, this, "gditimers", "Print installed GDI timers and callbacks", true);
plugin_imageinfo = new Analysis_Plugin_EXECUTION(null, this, "imageinfo", "Identify information for the image", true);
plugin_kdbgscan = new Analysis_Plugin_EXECUTION(null, this, "kdbgscan", "Search for and dump potential KDBG values", true);
//plugin_kpcrscan = new Analysis_Plugin_EXECUTION(null, this, "kpcrscan", "Search for and dump potential KPCR values", true);
plugin_lsadump = new Analysis_Plugin_EXECUTION(null, this, "lsadump", "Dump (decrypted) LSA secrets from the registry", true);

//plugin_memmap = new Analysis_Plugin_EXECUTION(null, this, "memmap", "Print the memory map", true);

plugin_messagehooks = new Analysis_Plugin_EXECUTION(null, this, "messagehooks", "List desktop and thread window message hooks", true);

//plugin_impscan = new Analysis_Plugin_EXECUTION(null, this, "impscan", "Scan for calls to imported functions", true);
plugin_joblinks = new Analysis_Plugin_EXECUTION(null, this, "joblinks", "Print process job link information", true);
plugin_notepad = new Analysis_Plugin_EXECUTION(null, this, "notepad", "List currently displayed notepad text", true);
plugin_sessions = new Analysis_Plugin_sessions(null, this, "sessions", "List details on _MM_SESSION_SPACE (user logon sessions)", true);
plugin_ssdt = new Analysis_Plugin_EXECUTION(null, this, "ssdt", "Display SSDT entries", true);

plugin_symlinkscan = new Analysis_Plugin_EXECUTION(null, this, "symlinkscan", "Pool scanner for symlink objects", true);
plugin_thrdscan = new Analysis_Plugin_EXECUTION(null, this, "thrdscan", "Pool scanner for thread objects", true);


//plugin_unloadedmodules = new Analysis_Plugin_EXECUTION(null, this, "unloadedmodules", "Print list of unloaded modules", true);
plugin_userassist = new Analysis_Plugin_user_assist(null, this, "userassist", "Print userassist registry keys and information", true);
plugin_printkey = new Analysis_Plugin_print_key(null, this, "printkey", "Print a registry key, and its subkeys and values", true);
plugin_userhandles = new Analysis_Plugin_EXECUTION(null, this, "userhandles", "Dump the USER handle tables", true);
plugin_vadinfo = new Analysis_Plugin_VAD_INFO(null, this, "vadinfo", "Dump the VAD info", true);
plugin_vadtree = new Analysis_Plugin_vadtree(null, this, "vadtree", "vadtree", true);
plugin_vadwalk = new Analysis_Plugin_EXECUTION(null, this, "vadwalk", "Walk the VAD tree", true);
if(PROFILE.toLowerCase().trim().startsWith("win10"))
	plugin_win10cookie = new Analysis_Plugin_EXECUTION(null, this, "win10cookie", "Find the ObHeaderCookie value for Windows 10", true);
plugin_windows = new Analysis_Plugin_EXECUTION(null, this, "windows", "Print Desktop Windows (verbose details)", true);
plugin_wintree = new Analysis_Plugin_EXECUTION(null, this, "wintree", "Print Z", true);
plugin_wndscan = new Analysis_Plugin_EXECUTION(null, this, "wndscan", "Pool scanner for window stations", true);

plugin_shellbags = new Analysis_Plugin_EXECUTION(null, this, "shellbags", "Prints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags info", true);
plugin_shimcache = new Analysis_Plugin_EXECUTION(null, this, "shimcache", "Parses the Application Compatibility Shim Cache registry key", true);
plugin_timeliner = new Analysis_Plugin_EXECUTION(null, this, "timeliner", "Creates a timeline from various artifacts in memory", false);

				//
				//VERINFO
				//				
				plugin_verinfo = new Analysis_Plugin_verinfo(null, this, "verinfo", "Prints out the version information from PE images", false);
				...
				plugin_cmdline = new Analysis_Plugin_cmdline(null, this, "cmdline", "Display process command", false);
				plugin_consoles = new Analysis_Plugin_consoles(null, this, "consoles", "Extract command history by scanning for _CONSOLE_INFORMATION", false);
				...
				/////////////////////////////////////////////////////////////////////
					...
					if(PROFILE.toLowerCase().contains("winxp") || PROFILE.toLowerCase().contains("2003"))
					{
						...
						plugin_connections = new Analysis_Plugin_connections(null, this, "connections", "Print list of open connections [Windows XP and 2003 Only]", false);
						
						//
						//connscan
						//
						plugin_connscan = new Analysis_Plugin_connscan(null, this, "connscan", "Pool scanner for tcp connections", false);
						
						//
						//sockets
						//
						plugin_sockets = new Analysis_Plugin_sockets(null, this, "sockets", "Print list of open sockets", false);
						
						//
						//sockscan
						//
						plugin_sockscan = new Analysis_Plugin_sockscan(null, this, "sockscan", "Pool scanner for tcp socket objects", false);
					}
					else//everything above WinXP
					{
						...
						//netscan
						//
						plugin_netscan = new Analysis_Plugin_netscan(null, this, "netscan", "Scan a Vista (or later) image for connections and sockets", false);
					}
			...
			sop("\nBuilding handles listing...");
				...
				//Handles
				//
				plugin_handles = new Analysis_Plugin_handles(null, this, "handles", "Print list of open handles for each process", false);
