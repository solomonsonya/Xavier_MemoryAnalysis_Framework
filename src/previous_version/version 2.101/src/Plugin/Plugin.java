...
public class Plugin extends JPanel implements ActionListener
{
	...
	public volatile static LinkedList<Plugin> list_plugins = new LinkedList<Plugin>();
	...
	/**to indicate the original add index of this plugin; useful for when we're generating the plugin selection frame to know where to place this plugin if the user chooses to send it back to the preselection list*/
	public volatile int original_index = 0;
	
	
	
	
	//actinlisteners will be added in the JDialogClass
...
//					}
	...
	public static boolean analyze_image()
	{
		try
		{
			...
			for(Plugin plugin : tree_selected_plugins.values())
			{
				...
				Process_Plugin process = new Process_Plugin(plugin, plugin.plugin_name, plugin.plugin_description, null, null, plugin.jtaConsole.jtf.getText(), true, false, "", true);
	...
	public boolean execute_command()
	{
		try
		{
			...
			Process_Plugin process = new Process_Plugin(this, this.plugin_name, this.plugin_description, null, null, this.jtaConsole.jtf.getText(), true, false, "", true);
