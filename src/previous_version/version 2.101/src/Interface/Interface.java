			...
			for(File fle : list)
			{
				...
				if(list_plugin_autorun != null && list_plugin_autorun.size() > 0)
				{
					if(list_plugin_autorun.size() == 1)
						driver.directivesp("\nOnly 1 specified plugin to autorun: " + list_plugin_autorun.getFirst());
					else
						driver.directivesp("\n[" + list_plugin_autorun.size() + "] Specified plugins to autorun: " + list_plugin_autorun.getFirst());
															
					for(int i = 1; i < list_plugin_autorun.size(); i++)
						driver.directivesp(", " + list_plugin_autorun.get(i));
