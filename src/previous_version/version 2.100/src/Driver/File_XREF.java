		...
		try
		{
			...
			if(intrface.advanced_analysis_director == null) //and working directory path was not specified previously
			{
				...
				else if(selection == 2) //proceed here and execute respective plugins to enable XREF search
				{
					...
					//create processlist
					intrface.advanced_analysis_director.process_list(true);
				}
				else
				{
					//action was canceled
...
//				
				...
				if(intrface.execute_xref_search_plugins_if_import_files_are_missing)
				{
					...
					{						
						intrface.advanced_analysis_director.plugin_filescan = new Analysis_Plugin_EXECUTION(null, intrface.advanced_analysis_director, "filescan", "Pool scanner for file objects", false);
						filescan = intrface.advanced_analysis_director.plugin_filescan.fleOutput;															
					}
					
					//handles
					if(!intrface.is_valid_file(handles))
						intrface.advanced_analysis_director.plugin_handles = new Analysis_Plugin_handles(null, intrface.advanced_analysis_director, "handles", "Print list of open handles for each process", false);
						
					//timeliner
					if(!intrface.is_valid_file(timeliner))
						intrface.advanced_analysis_director.plugin_timeliner = new Analysis_Plugin_EXECUTION(null, intrface.advanced_analysis_director, "timeliner", "Creates a timeline from various artifacts in memory", false);
					...
					if(!intrface.is_valid_file(userassist))
						intrface.advanced_analysis_director.plugin_userassist = new Analysis_Plugin_user_assist(null, intrface.advanced_analysis_director, "userassist", "Print userassist registry keys and information", false);
						
					//shellbags
					if(!intrface.is_valid_file(shellbags))
						intrface.advanced_analysis_director.plugin_shellbags = new Analysis_Plugin_EXECUTION(null, intrface.advanced_analysis_director, "shellbags", "Prints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags infoPrints ShellBags info", false);
					
					//shimcache
					if(!intrface.is_valid_file(shimcache))
						intrface.advanced_analysis_director.plugin_shimcache = new Analysis_Plugin_EXECUTION(null, intrface.advanced_analysis_director, "shimcache", "Parses the Application Compatibility Shim Cache registry key", false);	
						
					//svcscan
					if(!intrface.is_valid_file(svcscan))
						intrface.advanced_analysis_director.plugin_svcscan = new Analysis_Plugin_svcscan(null, intrface.advanced_analysis_director, "svcscan", "Scan for Windows services", false);
						
					//joblink
					if(!intrface.is_valid_file(joblink))
						intrface.advanced_analysis_director.plugin_joblinks = new Analysis_Plugin_EXECUTION(null, intrface.advanced_analysis_director, "joblinks", "Print process job link information", false);

					//vadinfo
					if(!intrface.is_valid_file(vadinfo))
						intrface.advanced_analysis_director.plugin_vadinfo = new Analysis_Plugin_VAD_INFO(null, intrface.advanced_analysis_director, "vadinfo", "Dump the VAD info", false);
						
					//verinfo
					if(!intrface.is_valid_file(verinfo))
						intrface.advanced_analysis_director.plugin_verinfo = new Analysis_Plugin_verinfo(null, intrface.advanced_analysis_director, "verinfo", "Prints out the version information from PE images", false);
						
										
					//moddump
					if(!intrface.is_valid_file(moddump))
						intrface.advanced_analysis_director.plugin_moddump = new Analysis_Plugin_SUPER_MODULES(null, intrface.advanced_analysis_director, "moddump", "Dump a kernel driver to an executable file sample", false); //runs modules and modscan within moddump!
											
						
					//dlllist
					if(!intrface.is_valid_file(dlllist))
						intrface.advanced_analysis_director.plugin_dlllist = new Analysis_Plugin_dlllist(null, intrface.advanced_analysis_director, "dlllist", "Print list of loaded dlls for each process", false);
						
					//dumpfiles evt
					if(!intrface.is_valid_file(dumpfiles))
						intrface.advanced_analysis_director.plugin_evtlogs = new Analysis_Plugin_EXECUTION(null, intrface.advanced_analysis_director, "dumpfiles --regex .evtx$ --ignore-case", "Dump Windows Event Logs ", false);
...
//					}
				...
				//yarascan
				///////////////////////////////
				if(intrface.jcb_IncludeYaraScan != null && intrface.jcb_IncludeYaraScan.isSelected())
				{
					//notify
					try	{ intrface.jtaFile_XREF_Search_Results.append("\n\nYARASEARCH\n" + driver.UNDERLINE);	} catch(Exception e){}
					
					//search
					plugin_yarascan = new Analysis_Plugin_YaraScan(null, intrface.advanced_analysis_director, "yarascan", "Scan process or kernel memory with Yara signatures", true, XREF_SEARCH_STRING, Start.intface.jtaFile_XREF_Search_Results, this);
				}
			}
	...
	public long search_file(File fle, String plugin_name, String search_string, String search_string_lower, int index_dump_files_processing_action)
	{
		...
		{						
			...
			{
				intrface.sop("Empty file received. No search actions for plugin/output file [" + plugin_name + "]");
				return -1;
			}
