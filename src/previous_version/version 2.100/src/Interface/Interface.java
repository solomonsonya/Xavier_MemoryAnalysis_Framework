...
public class Interface extends Thread implements Runnable, ActionListener
{
	...
	public File_XREF file_xref = null;
	...
	public static JPanel jpnlFile_XREF_MAIN = null;
		public static JPanel jpnlFile_XREF_Search_String_NORTH_Container = null;
			...
			public static JCheckBox jcb_IncludeYaraScan = null;
	...
	public boolean initialize_component()
	{
		try
		{
			...
			jmnuFile.add(jmnuitm_Import_Directory);	jmnuitm_Import_Directory.setEnabled(false);
			...
			jmnuFile.add(jmnuitm_SpecifyTimeZone);
			jmnuFile.add(jmnuitm_Specify_Investigation_Output_Directory);
			jmnuFile.add(jmnuitm_Enable_Search_TF_XREF);
			...
			this.jcb_IncludeYaraScan = new JCheckBox("Enable YARA Scan", false);
			this.jcb_RestrictSearch_to_Core_XREF_Plugins = new JCheckBox("Only search core XREF plugins", false);
			jpnl_jcb_search_string_options = new JPanel();
			
			//this.jtaFile_XREF_Search_Results.jpnlcheckBox.add(jcb_RestrictSearch_to_Core_XREF_Plugins);
			jpnlFile_XREF_Search_String_NORTH_Container.add(BorderLayout.EAST, jcb_IncludeYaraScan);
	...
	public boolean import_config_file()
	{
		try
		{
			...
			{
				driver.sop("Note: I could not file setup.conf file at " + Start.fleImportMemoryImageDirectory);
				return false;
			}
			...
			for(File fle : list)
			{
				...
				if(list_plugin_autorun != null && list_plugin_autorun.size() > 0)
				{
					if(list_plugin_autorun.size() == 1)
						sp("Only 1 specified plugin to autorun: " + list_plugin_autorun.getFirst());
					else
						sp("[" + list_plugin_autorun.size() + "] Specified plugins to autorun: " + list_plugin_autorun.getFirst());
					
					for(int i = 1; i < list_plugin_autorun.size(); i++)
						sp(", " + list_plugin_autorun.get(i));
		...
		try
		{
			...
			for(Node_Generic node : File_XREF.tree_dump_file_entries_FILEDUMP_XREF.values())
			{
				try
				{
					...
					if(process_memdump)
					{
						node.plugin_execution =  new Analysis_Plugin_memdump(null, advanced_analysis_director, "memdump", "Dump the addressable memory for a process", true, node.pid);
					}
					else	//else, FILESCAN dump file!
						node.plugin_dumpfile = new Analysis_Plugin_DumpFiles(null, advanced_analysis_director, "dumpfiles -Q " + node.offset_p, "Dump Windows Event Logs ", true, node.file_name, node.offset_p);
