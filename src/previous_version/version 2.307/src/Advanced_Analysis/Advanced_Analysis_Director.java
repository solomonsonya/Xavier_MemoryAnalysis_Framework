		...
		try
		{
			...
			for(File fle_setup_conf : list_files)
			{
				...
				for(File fle : list)
				{
					...
					//
					if(fle != null && fle.getCanonicalPath().toLowerCase().trim().endsWith(".exe"))
					{
						sop("\n* * NOTE: Although I found setup.conf file at " + fle_setup_conf + "\n\t I see there is a .exe file which I will not include as part of this analysis. \nRename [" + fle.getName() + "] to anything else that does not end in .exe if you truly wish to analyze that file as well");
						continue;
					}
					
					unwanted_extension = ".jar";
				...
				driver.directive("In " + myClassName + ", ready to analyze \n\n" + fle_setup_conf.getCanonicalPath() + "\n\t" + fle_image.getCanonicalPath() + "\n\n");
				
			}

			driver.jop("NOTE: This feature is not ready for implementation!");
