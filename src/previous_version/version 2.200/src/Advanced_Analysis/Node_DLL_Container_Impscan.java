...
public class Node_DLL_Container_Impscan 
{
	...
	public static boolean import_manifest_line_entry_NODE_IS_ENTIRE_LINE(String mtd_designator, String line, Advanced_Analysis_Director director, Node_Process process)
	{
		try
		{
			...
			if(director == null && process == null)
			{
				driver.directive("* * * * ERROR! Director and process are both null! I am unable to complete import_manifest_line_entry mtd_designator: [" + mtd_designator + "] in class: " + myClassName + "\t line --> " + line);
				return false;
			}
			...
			String function_name_lower = null;
			
			for(int i = 0; i < arr.length; i+=2)
			{
				try
				{
					if(arr[i].toLowerCase().trim().equals("module_name"))
						module_name = arr[i+1].trim();
					else if(arr[i].toLowerCase().trim().equals("module_name_lower"))
						module_name_lower = arr[i+1].trim();
					else if(arr[i].toLowerCase().trim().equals("iat"))
						IAT = arr[i+1].trim();
					else if(arr[i].toLowerCase().trim().equals("call"))
						call = arr[i+1].trim();
					else if(arr[i].toLowerCase().trim().equals("function_name"))
						function_name = arr[i+1].trim();
					else if(arr[i].toLowerCase().trim().equals("function"))
						function_name = arr[i+1].trim();
					else if(arr[i].toLowerCase().trim().equals("function_name_lower"))
						function_name_lower = arr[i+1].trim();
					else
						driver.directive("Error in import_manifest_line_at index [" + i + "] Unknown key: [" + arr[i] + "] while parsing line --> " + line );
					
				}
				catch(Exception e)
				{
					driver.directive("Error in import_manifest_line_at index [" + i + "] while parsing line --> " + line );
					continue;
				}									
			}
			
			//
			//Part 2: Ensure we have tree established
			//
			if(process.tree_impscan_DLL_containers == null)
				process.tree_impscan_DLL_containers = new TreeMap<String, Node_DLL_Container_Impscan>();
				
				
			//
			//Part 3: Process entries
			//
			Node_DLL_Container_Impscan dll = new Node_DLL_Container_Impscan(process, module_name, IAT, call, function_name);
