			...
			if(Interface.advanced_analysis_director != null && Interface.advanced_analysis_director.tree_advanced_analysis_threads != null)
			{
				...
				if(plugins_in_execution < 1)
				{
					if(Interface.jlblStatusMessage.isVisible())
					{
						...
						{
							Start.intface.advanced_analysis_director.execute_completion_actions();
								
							if(Start.intface.launching_auto_advanced_analysis && Start.intface.self_terminate_upon_execution)
							{
								driver.directive("Autostart was enabled and self terminate upon execution completion. Program Terminated.");
								System.exit(0);
							}
						}
