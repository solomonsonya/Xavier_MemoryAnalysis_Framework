...
public class FileAttributeData 
{
	...
	public FileAttributeData(Node_DLL dll)
	{
		try
		{
			this.DLL = dll;
		...
		{
			driver.eop(myClassName, "Constructor - 6 on DLL", e);
		}
	}
	...
	/**
	 * import for function: write_manifest_entry. header == e.g., file_attr. this fcn will seek this location first, trim all header to this point, then will begin analyzing just the key and value
	 * @param pw
	 * @param header
	 * @param override_file_name
	 * @return
	 */
	public boolean import_manifest_entry(String key, String value, String line)
	{
		try
		{
			...
			if(line != null && line.toLowerCase().contains("file_attributes"))
			{
				line = line.substring(line.indexOf("file_attributes") + "file_attributes".length()).trim();
				...
				{
					driver.directive("\n\n* * * ERROR! I could not bifurcate \"file_attributes\" and establish new key and value from line --> " + line);
				}
				
			}
			
			if(line != null && line.toLowerCase().contains("file_attr:"))
			{
				line = line.substring(line.indexOf("file_attr:") + "file_attr:".length()).trim();
			...
			if(key.startsWith("raw_file_length"))					
			{
				try	{	raw_file_length = Long.parseLong(value);}
				catch(Exception e)
				{
					driver.directive("\n\n* ** ERROR! I was unale to parse raw_file_length given key[" + key + "] value[" + value + "]");
				}
			}
			...
			else if(key.startsWith("length"))					
			{
				...
				{
					driver.directive("\n\n* ** ERROR! I was unale to parse length given key[" + key + "] value[" + value + "]");
				}
			}
			
			else if(key.startsWith("file_name"))					file_name = value;
			else if(key.startsWith("size"))							size = value;
			else if(key.startsWith("creation_date"))				creation_date = value;
			else if(key.startsWith("last_accessed"))				last_accessed = value;
			else if(key.startsWith("last_modified"))				last_modified = value;
			else if(key.startsWith("hash_md5"))						hash_md5 = value;
			else if(key.startsWith("hash_sha256"))					hash_sha256 = value;
			else if(key.startsWith("is_hashing_complete") && value.toLowerCase().startsWith("true"))					is_hashing_complete = true; 
			else if(key.startsWith("attributes:"))					attributes = line.substring("attributes:".length()).trim();
			else if(key.startsWith("attributes"))					attributes = line.substring("attributes".length()).trim();
			else if(key.startsWith("short_file_name"))				short_file_name = value;
			
			else if(key.startsWith("is_file") && value.toLowerCase().startsWith("true"))					is_file = true;
			else if(key.startsWith("is_directory") && value.toLowerCase().startsWith("true"))					is_directory = true;
			
			else
			{
				throw new Exception("Parse Error! Unknown key and value pair!");
			}
		...
		{
			driver.directive("Unknown token received in " + this.myClassName + "! I could not parse key: [" + key + "] value: [" + value + "]");
