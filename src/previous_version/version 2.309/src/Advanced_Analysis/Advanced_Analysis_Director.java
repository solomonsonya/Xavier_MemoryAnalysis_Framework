...
public class Advanced_Analysis_Director extends Thread implements Runnable 
{
	...
	public volatile File fle_directory_of_images_to_analyze = null;
	...
	public volatile LinkedList<Tuple> list_advanced_analysis_file_tuples = null;
	...
	public volatile JTextArea_Solomon jtaMalfind = null;
...
//public volatile TreeMap<String, Analysis_Plugin_impscan> tree_executed_imports = new TreeMap<String, Analysis_Plugin_impscan>();
	...
	/**Keep track of malfind nodes that test positive to have MZ present which could be indicative of process hooking e.g. meterpreter*/
	public volatile TreeMap<String, Node_Malfind> tree_malfind_MZ_Detcted = new TreeMap<String, Node_Malfind>();
	public volatile TreeMap<String, Node_Malfind> tree_malfind_JMP_Detcted = new TreeMap<String, Node_Malfind>();
	...
	public Advanced_Analysis_Director(Interface INTERFACE, File analysis_directory_ok_to_be_null)
	{
		try
		{
			intrface = INTERFACE;
			
			fle_directory_of_images_to_analyze = analysis_directory_ok_to_be_null;
			
			if(fle_directory_of_images_to_analyze == null || !fle_directory_of_images_to_analyze.isDirectory())
			{
				fle_directory_of_images_to_analyze = driver.querySelectFile(true, "Please select directory containing memory images to analyze", JFileChooser.DIRECTORIES_ONLY, null);
			}
			
			if(fle_directory_of_images_to_analyze != null && fle_directory_of_images_to_analyze.exists())
				this.commence_action();
			else
				driver.directive("Halting actions to analyze directory of images - no valid directory was selected or user canceled action");
		}
		catch(Exception e)
		{
			
			driver.eop(myClassName, "Constructor - 5 analyze directory of images", e);
		}
...
//			}//end else
	 ...
	 * @return
	 */
	public boolean instantiate_analysis_across_directory_of_memory_images(File directory)
	{
		try
		{
			if(directory == null || !directory.isDirectory())
			{
				driver.directive("PUNT! Invalid directory received! Halting actions here...");
				return false;
			}
			...
			sop("Initiating analysis on directory " + directory.getCanonicalPath());
			
			list_advanced_analysis_file_tuples = new LinkedList<Tuple>();
			
			LinkedList<File> list_files = new LinkedList<File>();
			
			list_files = driver.list_files(directory, true, new String[]{"setup.conf"}, list_files);
			
			if(list_files == null || list_files.isEmpty())
			{
				sop("Error! I am unable to continue. No setup.conf files were found under top folder --> " + directory.getCanonicalPath() + "\nRestart this process if necessary...");
				return false;
			}
			
			//iterate through each setup.conf file and analysis directory
			for(File fle_setup_conf : list_files)
			{
				if(fle_setup_conf == null || !fle_setup_conf.isFile())
					continue;
				
				//Find image file to analyze
				LinkedList<File> list = new LinkedList<File>(); 
				list = driver.getFileListing(fle_setup_conf.getParentFile(), false, null, list);
				
				if(list == null || list.isEmpty())
				{
					sop("\n* PUNT! Although I found setup.conf file at " + fle_setup_conf + "\n\tI could not locate an image file within the same directory. I am skipping analysis of " + fle_setup_conf);
					continue;
				}
				...
				String [] arr_reject = new String[]
						{
								".exe", ".jar", ".py", ".txt", ".docx", ".doc", ".ppt", ".pptx", 
						};
				
				for(File fle : list)
				{
					if(fle == null || !fle.isFile() || fle.getCanonicalPath().toLowerCase().trim().endsWith(".conf") || fle.getCanonicalPath().toLowerCase().trim().endsWith(".config") || fle.getCanonicalPath().toLowerCase().trim().endsWith(".configuration"))
						continue;
					...
					for(String unwanted_extension : arr_reject)
					{
						if(fle != null && fle.getCanonicalPath().toLowerCase().trim().endsWith(unwanted_extension))
						{						
							sop("\n* * NOTE: Although I found setup.conf file at " + fle_setup_conf + "\n\t I see there is a " + unwanted_extension + " file which I will not include as part of this analysis. \n\tRename [" + fle.getName() + "] to anything else that does not end in " + unwanted_extension + " if you truly wish to analyze that file as well.");
							keep_file = false;
							break;
						}
					}
					...
					//otw, take the first image found that is not setup.conf
					fle_image = fle;
					break;
				}
				
				//validate
				if(fle_image == null || !fle_image.isFile())
				{
					sop("\n* * * * PUNT! Although I found setup.conf file at " + fle_setup_conf + "\n\tI could not locate an image file within the same directory. I am skipping analysis of " + fle_setup_conf.getParentFile().getCanonicalPath());
					continue;
				}
				
				Tuple tuple = new Tuple(fle_setup_conf, fle_image);
				list_advanced_analysis_file_tuples.add(tuple);
				
				//call_advanced_analysis_director(fle_setup_conf, fle_image);
				
			}// end for

			return call_advanced_analysis_director();
		}
		catch(Exception e)
		{
			driver.eop(myClassName, "instantiate_analysis_across_directory_of_memory_images", e);
		}
	...
	/**
	 * recursive fcn, call self until list is depleted
	 * @return
	 */
	public boolean call_advanced_analysis_director()
	{
		try
		{
			//
			//base case
			//
			if(this.list_advanced_analysis_file_tuples == null || list_advanced_analysis_file_tuples.isEmpty())
			{
				sop("\n\n*\n\t*\n\t\t*\n\t\t\t PROCESS COMPLETE!!!\n\t\t*\n\t*\n*");
				return true;
			}
			...
			//
			Tuple tuple = list_advanced_analysis_file_tuples.removeFirst();
			
			//notify
			try	{	driver.directive("\nRecursing to next advanced analysis call. Number of remaining calls after this one: [" + list_advanced_analysis_file_tuples.size() + "]");	}	catch(Exception e){}
			
			//call execution function
			call_advanced_analysis_director(tuple.file_1, tuple.file_2);
			
			//fall through to recurse to next element			
		}
		catch(Exception e)
		{
			driver.eop(myClassName, "call_advanced_analysis_director - recursive caller", e);
		}
	...
	public boolean call_advanced_analysis_director(File fle_setup_conf, File fle_image)
	{
		try
		{
			if(fle_setup_conf == null || fle_image == null)
			{
				driver.sop("\n* * * ERROR! I am unable to instantiate one of the Analysis Directors due to invalid import file:\n\tSetup.Conf file: " + fle_setup_conf + "\n\tFile Image: " + fle_image);
				return false;
			}
			
			//ensure we have launch path to jar file
			if(Driver.fle_launch_file == null || !Driver.fle_launch_file.isFile())
			{
				driver.set_launch_executable();
			}
			...
			if(driver.isWindows && (Start.intface.fle_volatility == null || !Start.intface.fle_volatility.isFile()))
			{
				driver.jop("Please select valid volatility executable in order to use this feature!");
				return false;
			}
			...
			fleOutput = new File(Interface.path_fle_analysis_directory + "advanced_analysis" + File.separator + "_log_" + fle_image.getName() + ".txt");
			...
			JTextArea_Solomon jta = null;
			
			try	{	jta = Interface.jpnlAdvancedAnalysisConsole;	} catch(Exception e){}
			...
			String execution_command = "", command = "";
			boolean auto_terminate = true;
			
			
			command = "java -jar \"" + Driver.fle_launch_file.getCanonicalPath().trim() + "\" -a " +"\"" + Start.intface.fle_volatility.getCanonicalPath() + "\" " + "\"" + fle_setup_conf.getCanonicalPath() + "\" " + "\"" + fle_image.getCanonicalPath() + "\" " + auto_terminate;
						
			jta.append("\n" + Driver.UNDERLINE + "\nInstantiating Advanced Analysis on [" + fle_image.getName() + "]\npath to image: " + fle_image.getCanonicalPath() + "\npath to setup.conf: " + fle_setup_conf.getCanonicalPath());
			
			
			driver.directive("\n" + Driver.UNDERLINE + "\nInstantiating Advanced Analysis on [" + fle_image.getName() + "]\npath to image: " + fle_image.getCanonicalPath() + "\npath to setup.conf: " + fle_setup_conf.getCanonicalPath());
			//driver.directive(command);
			
			if(driver.isWindows)
			{
				process_builder = new ProcessBuilder("cmd.exe", "/C",  command);							
			}
							
			//else if(driver.isLinux)
			else
			{
				process_builder = new ProcessBuilder("/bin/bash", "-c",  command);
			}			
			...
			PrintWriter pwCmd = new PrintWriter(new BufferedOutputStream(process.getOutputStream()));
			
			
			long line_count = 0;
			
			pw.println("Instantiating Advanced Analysis on [" + fle_image.getName() + "]");
			pw.println("path to image: " + fle_image.getCanonicalPath());
			pw.println("path to setup.conf: " + fle_setup_conf.getCanonicalPath());
			pw.println("Timestamp: " + driver.get_time_stamp());
			pw.println(Driver.UNDERLINE + "\n");
		    ...
		    {
		    	driver.sop("check plugin process execution " + "call_advanced_analysis_director" + " - " + command);
		    }
		   ...
		   //clean up
			...
			sop("\nExecution complete for file: " + fle_image.getCanonicalPath());		
		...
		{
			driver.eop(myClassName, "call_advanced_analysis_director", e);
		}
	...
	public boolean commence_action()
	{
		...
		{			
			...
			// Load manifest file!
			//////////////////////////////////////////////////////////////////
			if(fle_directory_of_images_to_analyze != null)
			{
////////////////////////////////////////////////////////////
				Thread thread_execution = new Thread() 
				{
					public void run() 
					{
						try 
						{
							instantiate_analysis_across_directory_of_memory_images(fle_directory_of_images_to_analyze);						
						} 
...
//////////////////////////////////////////////////////////////////
			...
			// Load manifest file!
			//////////////////////////////////////////////////////////////////
			else if(this.fle_manifest_IMPORT != null)
			{
...
//				this.write_manifest(WRITE_MANIFEST_DELIMITER);
	...
	public boolean analyze_malfind_entries()
	{
		try
		{
			if( (this.tree_malfind_MZ_Detcted == null || this.tree_malfind_MZ_Detcted.size() < 1) && (this.tree_malfind_JMP_Detcted == null || this.tree_malfind_JMP_Detcted.size() < 1) )
				return false;
			
			//notify
			this.sop("\n* * * Specific Malfind entries were detected that require your attention" );
			
			//plugin	process id	parent id	
			
			if(jtaMalfind == null)
			{
				jtaMalfind = new JTextArea_Solomon("", true, "Malfind Entries Requiring Attention - TSV", false);				
				Start.intface.populate_export_btn(jtaMalfind);
				...
				jtabbed_pane.addTab("Malfind Entries Requiring Attention", jtaMalfind);	
				
				try	{	Start.intface.jtabbedpane_AdvancedAnalysis.setToolTipTextAt(1, "<html>Entries are Tab-separated values<br>You can paste these entries for instance into an Excel type spreadsheet, <br> and filter by specific entries.  </html>");} catch(Exception e){}
			...
			try
			{
				...
				for(Node_Malfind malfind : this.tree_malfind_MZ_Detcted.values())
				{
					culprit_line = malfind.culprit_line;
					
					if((culprit_line == null || culprit_line.trim().equals("")) && malfind.list_details != null)
					{
						//get culprit line
						for(String line : malfind.list_details)
						{
							if(line == null)
								continue;

							String line_analysis = line.replace("     ", " ").replace("    ", " ").replace("   ", " ").replace("  ", " ");
							...
							if(arr != null && arr.length > 0)
							{
								...
								try
								{
									if( (arr[1].toLowerCase().trim().equals("4d") || arr[1].toLowerCase().trim().equals("5a")) && (arr[2].toLowerCase().trim().equals("5a") || arr[2].toLowerCase().trim().equals("4d")) )
									{
					...
					jtaMalfind.append("malfind" + delimiter + malfind.pid + delimiter + malfind.process_name + delimiter + malfind.address + delimiter + malfind.vad_tag + delimiter + malfind.protection + delimiter + malfind.flags + delimiter + malfind.MZ_present + delimiter + malfind.Trampoline_initial_JMP_Detected +  delimiter + culprit_line);
			...
			try
			{
				...
				for(Node_Malfind malfind : this.tree_malfind_JMP_Detcted.values())
				{
					culprit_line = malfind.culprit_line;
					
					if((culprit_line == null || culprit_line.trim().equals("")) && malfind.list_details != null)
					{
						if(malfind.list_details.get(5).toLowerCase().contains("jmp "))
						{
		...
		{
			driver.eop(myClassName, "analyze_malfind_entries", e);
		}
				...
				{						
					...
					{									
						...
						if(mz_present != null && mz_present.startsWith("true"))
						{
							...
							tree_malfind_MZ_Detcted.put(line + " MZ", node_malfind_import_manifest);
						}
						...
						if(trampoline_initial_JMP_detected != null && trampoline_initial_JMP_detected.startsWith("true"))
						{
							...
							tree_malfind_JMP_Detcted.put(line + " JMP", node_malfind_import_manifest);
						}
