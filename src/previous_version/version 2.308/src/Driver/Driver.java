...
import Plugin.Process_Plugin;
	...
	public class Driver 
	{
		...
		public static final String VERSION = "2.308";
		public static final String FULL_NAME = NAME + " vrs " + VERSION;
						
		public static volatile File fle_launch_file = null;
		public static volatile String launch_path = null;
		...
		public static boolean map_launch_path()
		{
			try
			{
				 String execution_binary = Driver.class.getProtectionDomain().getCodeSource().getLocation().toURI().getPath();
				 
				 if(execution_binary == null)
					 return false;
				 
				 execution_binary = execution_binary.trim();
				 
				 if(execution_binary.startsWith("/") || execution_binary.startsWith("\\"))
					 execution_binary = execution_binary.substring(1).trim();
				 
				 fle_launch_file = new File(execution_binary);
		         
				 if(fle_launch_file != null && fle_launch_file.isFile())
					 Driver.launch_path = fle_launch_file.getParentFile().getCanonicalPath().trim();
				 else
					 Driver.launch_path = fle_launch_file.getCanonicalPath();
				 
				 if(Driver.launch_path != null && !Driver.launch_path.trim().endsWith(File.separator))
					 Driver.launch_path = Driver.launch_path.trim() + File.separator;
			...
			{
				System.out.println("Check " + myClassName + " map_launch_path mtd");
			}
	...
	public File set_launch_executable()
	{
		try
		{
			File fle = this.querySelectFile(true, "I couldn't find my launch file. Please select launch executable, e.g., xavier_framework.jar", JFileChooser.FILES_ONLY, null);
			
			if(fle == null || !fle.isFile())
			{
				sop("Punt! No valid file selected!");
				return null;
			}
			
			Driver.fle_launch_file = fle;
		...
		{
			System.out.println("Check set_launch_executable mtd in class: " + myClassName);
		}
		
		return Driver.fle_launch_file;
	}
