...
//below - was working with dependencies import file, however I'll replace with impscan for simplicity and to deprecate dependency on Dependencies file
	...
	public boolean search_XREF(String search_string, String search_string_lower, JTextArea_Solomon jta)
	{
		try
		{
			...
			//parent process name
			if(this.parent_process != null && this.parent_process.get_process_html_header().toLowerCase().contains(search_string_lower))
				append_to_jta_XREF("Parent Process: " + parent_process.get_process_html_header(), jta);
			
			//search own process name
			if(this.get_process_html_header().toLowerCase().contains(search_string_lower))
			...
			try
			{
				if(command_line != null && command_line.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Command Line: " + this.command_line, jta);
				if(path != null && path.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Path: " + this.path, jta);
				if(threads  != null && threads .toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Thread Count: " + this.threads , jta);
				if(handles != null && handles.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Handle Count: " + this.handles, jta);
				if(wow64 != null && wow64.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Wow64: " + this.wow64, jta);
				if(session != null && session.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Session: " + this.session, jta);
				if(offset_pslist != null && offset_pslist.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSList Offset: " + this.offset_pslist, jta);
				if(PDB != null && PDB.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PDB: " + this.PDB, jta);
				if(time_created_date != null && time_created_date.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Creation Date: " + this.time_created_date, jta);
				if(time_created_time != null && time_created_time.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Creation Time: " + this.time_created_time, jta);
				if(time_created_UTC != null && time_created_UTC.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("UTC Creation Time: " + this.time_created_UTC, jta);
				if(time_exited_date != null && time_exited_date.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Date Exited: " + this.time_exited_date, jta);
				if(time_exited_time != null && time_exited_time.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("Time Exited: " + this.time_exited_time, jta);
				if(time_exited_UTC != null && time_exited_UTC.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("UTC Time Exited: " + this.time_exited_UTC, jta);
				if(offset_psscan != null && offset_psscan.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSScan Offset: " + this.offset_psscan, jta);
				if(offset_pstree != null && offset_pstree.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSTree Offset: " + this.offset_pstree, jta);
				if(offset_psxview != null && offset_psxview.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSXview Offset: " + this.offset_psxview, jta);
				if(psxview_pslist != null && psxview_pslist.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSXview - Pslist: " + this.psxview_pslist, jta);
				if(psxview_psscan != null && psxview_psscan.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSXview - PSScan: " + this.psxview_psscan, jta);
				if(psxview_thrdproc != null && psxview_thrdproc.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSXview - thrdproc: " + this.psxview_thrdproc, jta);
				if(psxview_pspcid != null && psxview_pspcid.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSXview - pspcid: " + this.psxview_pspcid, jta);
				if(psxview_csrss != null && psxview_csrss.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSXview - csrss: " + this.psxview_csrss, jta);
				if(psxview_session != null && psxview_session.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSXview - session: " + this.psxview_session, jta);
				if(psxview_deskthrd != null && psxview_deskthrd.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("PSXview - deskthrd: " + this.psxview_deskthrd, jta);
				if(offset_V_dlldump  != null && offset_V_dlldump .toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("DLLDump - Offset (V): " + this.offset_V_dlldump , jta);
				if(offset_P_dlldump_trimmed  != null && offset_P_dlldump_trimmed .toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("DLLDump - Offset (P) - Trimmed: " + this.offset_P_dlldump_trimmed , jta);
				if(module_base_address_dlldump  != null && module_base_address_dlldump .toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("DLLDump Module Base Address: " + this.module_base_address_dlldump , jta);
				if(module_base_address_dlldump_trimmed != null && module_base_address_dlldump_trimmed.toLowerCase().trim().contains(search_string_lower)) this.append_to_jta_XREF("DLLDump Module Base Address - Trimmed: " + this.module_base_address_dlldump_trimmed, jta);
			}
			catch(Exception e)
			{
			...
			//////////////////////////////////////////////////////////
			if(VAD != null)
				VAD.search_XREF(search_string, search_string_lower, jta, this, "My VAD Info");
			...
			try
			{
				if(this.tree_handles != null && tree_handles.size() > 0)
				{
					//search values
					for(Node_Handle handle : tree_handles.values())
					{
						...
						if(handle.handle_value.toLowerCase().trim().contains(search_string_lower))
							append_to_jta_XREF("Handle Value: " + handle.handle_value, jta);
						if(handle.access_value.toLowerCase().trim().contains(search_string_lower))
							append_to_jta_XREF("Handle Access: " + handle.access_value, jta);
						if(handle.offset.toLowerCase().trim().contains(search_string_lower))
							append_to_jta_XREF("Handle Offset: " + handle.offset, jta);
						if(handle.type.toLowerCase().trim().contains(search_string_lower))
							append_to_jta_XREF("Handle Type: " + handle.type, jta);
					}
					
					
					//search keys
					for(String key : tree_handles.keySet())
					{
						if(key == null)
							continue;
						...
						if(key.contains(search_string_lower))
							append_to_jta_XREF("Handle Key: " + key, jta);
			...
			try
			{
				if(this.tree_dll != null && tree_dll.size() > 0)
				{
					for(Node_DLL dll : tree_dll.values())
					{
						...
						if(dll.search_XREF(search_string, search_string_lower, jta, false, this))
							XREF_SEARCH_HIT_FOUND = true;
			...
			try
			{
				if(this.tree_impscan_DLL_containers != null && this.tree_impscan_DLL_containers.size() > 0)
				{
					for(Node_DLL_Container_Impscan dll : this.tree_impscan_DLL_containers.values())
					{
						...
						{
							if(function_name != null && function_name.toLowerCase().trim().contains(search_string_lower))
							{
			...
			try
			{
				if(this.tree_privs != null)
				{
					for(String key : tree_privs.keySet())
					{
						if(key == null)
							continue;
						
						if(key.toLowerCase().trim().contains(search_string_lower))
							this.append_to_jta_XREF("Privilege: " + key, jta);
					}
				}
			}
			catch(Exception e)
			...
			try
			{
				if(this.tree_services_svcscan != null)
				{
					for(String key : tree_services_svcscan.keySet())
					{
						if(key == null)
							continue;
						
						if(key.toLowerCase().trim().contains(search_string_lower))
							this.append_to_jta_XREF("Service: " + key, jta);
					}
				}
			}
			catch(Exception e)
			...
			try
			{
				if(this.tree_environment_vars != null)
				{
					for(String key : tree_environment_vars.keySet())
					{
						if(key == null)
							continue;
						
						if(key.toLowerCase().trim().contains(search_string_lower))
							this.append_to_jta_XREF("Environment Var: " + key + " Value: " + tree_environment_vars.get(key).value, jta);
					}
				}
			}
			catch(Exception e)
			...
			try
			{
				...
				{										
					...
					for(String key : tree_malfind.keySet())
					{
						...
						//
						node.search_XREF(search_string, search_string_lower, jta, this);
					}
				}
			}
			catch(Exception e)
			...
			try
			{
				...
				{																	
					for(Node_Generic node : tree_vad_info.values())
					{
						...
						node.search_XREF(search_string, search_string_lower, jta, this, "VAD Info");
					}
				}
			}
			catch(Exception e)
