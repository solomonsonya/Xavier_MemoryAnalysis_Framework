...
public class Node_CmdScan 
{
	...
	public Node_CmdScan(Advanced_Analysis_Director parent)
	{
		try
		{
			director = parent;
	...
	public boolean import_manifest_line_entry(String line_entry, String []arr, String key, String value, Advanced_Analysis_Director DIRECTOR)
	{
		try
		{
			...
			//Particulars
			//
			if(key.equals("command_history_id"))
			{
				value = driver.trim_key(key, line_entry, true);								
				value_lower = value.toLowerCase().trim();
				
				this.command_history_id = value;
			}
			
			else if(key.equals("commandhistory"))
			{
				value = driver.trim_key(key, line_entry, true);								
				value_lower = value.toLowerCase().trim();
			...
			else if(key.equals("consoleprocess"))
			{
				value = driver.trim_key(key, line_entry, true);								
				value_lower = value.toLowerCase().trim();
				
				this.ConsoleProcess_line_1 = value;
			}
			
			else if(key.equals("console_id"))
			{
				value = driver.trim_key(key, line_entry, true);								
				value_lower = value.toLowerCase().trim();
				
				this.Console_id_line_2 = value;
			}
			
			else if(key.equals("historybuffercount"))
			{
				value = driver.trim_key(key, line_entry, true);								
				value_lower = value.toLowerCase().trim();
				
				this.HistoryBufferCount_line_3 = value;
			}
			
			else if(key.equals("originaltitle"))
			{
				value = driver.trim_key(key, line_entry, true);								
				value_lower = value.toLowerCase().trim();
				
				this.OriginalTitle_line_4 = value;
			}
			...
			else if(key.equals("title"))
			{
				...
				this.Title_line_5 = value;
			}
			
			else if(key.equals("list_cmd_header"))
			{
				value = driver.trim_key(key, line_entry, true);								
				value_lower = value.toLowerCase().trim();
				...
				{
					if(this.list_cmd_header == null)
						list_cmd_header = new LinkedList<String>();
					
					if(!value_lower.toLowerCase().equals("null"))
						list_cmd_header.add(value);
				}
				else
					throw new Exception("I could not find process attached to me!");
				
				
			}
			
			else if(key.equals("list_cmd_details"))
			{
				value = driver.trim_key(key, line_entry, true);								
				value_lower = value.toLowerCase().trim();
				...
				if(this.process != null)
				{
					if(this.list_cmd_details == null)
						list_cmd_details = new LinkedList<String>();
					
					if(!value_lower.toLowerCase().equals("null"))
						list_cmd_details.add(value);
				}
				else
					throw new Exception("* I could not find process attached to me!!!");
			...
			else if(key.equals("list_consoles_output"))
			{
				value = driver.trim_key(key, line_entry, false);								
				value_lower = value.toLowerCase().trim();
				
				if(this.process != null)
				{
					if(this.list_consoles_output == null)
						list_consoles_output = new LinkedList<String>();
					
					if(!value_lower.toLowerCase().equals("null"))
					{						
						if(line_entry.trim().length() < 1)
							list_consoles_output.add(".");
						else						
							list_consoles_output.add(line_entry.substring(20));
					}
				}
				else
					throw new Exception("* I could not find process attached to me!!!");
